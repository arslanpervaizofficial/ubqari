...
<div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
    ...
    <div class="bg-white p-5 rounded-xl shadow-sm"><div class="text-gray-500 text-sm">Total Due</div><div class="text-2xl font-bold text-red-600">{{ number_format($totalDue,2) }}</div></div>
    <div class="bg-white p-5 rounded-xl shadow-sm"><div class="text-gray-500 text-sm">Credit Balance</div><div class="text-2xl font-bold">{{ number_format($customer->credit_balance,2) }}</div></div>
...
{{--
    ...
    columns + per-row Delete forms below, and put the colspan back.

@if(auth()->user()->role === 'admin')
<div class="flex items-center gap-2 mb-3 print:hidden">
    <span data-bulk-count="customer-detail-orders" class="text-sm text-gray-500"></span>
    <button type="button" class="btn btn-red" data-bulk-submit="customer-detail-orders"
            data-action-url="{{ route('orders.destroy-selected') }}"
            data-confirm-message="Move the selected orders to Trash? Stock and this customer's balance will be adjusted.">
        Delete Selected
    </button>
</div>
@endif
--}}
<div data-ajax-list="customer-orders">
<div class="bg-white rounded-xl shadow-sm overflow-x-auto">
<h2 class="font-semibold p-4 border-b">Orders</h2>
<table class="w-full text-sm">
    <thead class="bg-gray-50 text-left"><tr>
        {{-- <th class="p-3"><input type="checkbox" data-bulk-select-all="customer-detail-orders"></th> --}}
        <th class="p-3">Order #</th><th class="p-3">Date</th><th class="p-3">Items</th><th class="p-3">Total</th><th class="p-3">Due</th><th class="p-3">Status</th>
        {{-- <th class="p-3">Actions</th> --}}
    </tr></thead>
    <tbody>
    @forelse($orders as $o)
        <tr class="border-t">
            {{-- <td class="p-3"><input type="checkbox" data-bulk-item="customer-detail-orders" value="{{ $o->id }}"></td> --}}
            ...
            {{--
            <td class="p-3">
                <form method="POST" action="{{ route('orders.destroy', $o) }}" class="inline confirm-submit" data-confirm-message="Move order {{ $o->order_number }} to Trash? Stock and this customer's balance will be recounted automatically.">
...
<h2 class="font-semibold p-4 border-b">Returns from this Customer</h2>
{{--
@if(auth()->user()->role === 'admin')
<div class="flex items-center gap-2 px-4 pt-3 print:hidden">
    <span data-bulk-count="customer-detail-returns" class="text-sm text-gray-500"></span>
    <button type="button" class="btn btn-red" data-bulk-submit="customer-detail-returns"
...
<table class="w-full text-sm">
    <thead class="bg-gray-50 text-left"><tr>
        {{-- <th class="p-3"><input type="checkbox" data-bulk-select-all="customer-detail-returns"></th> --}}
    ...
    @foreach($returns as $r)
        <tr class="border-t">
            {{-- <td class="p-3"><input type="checkbox" data-bulk-item="customer-detail-returns" value="{{ $r->id }}"></td> --}}
